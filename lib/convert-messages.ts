...
import type { ToolInvocation, Message as V4Message, UIMessage as LegacyUIMessage } from "./types/ai-sdk-v4-legacy";
import { MyUIMessage } from "./types/messages";
...
function isV4ReasoningPart(part: unknown): part is V4ReasoningPart {
  return (
    typeof part === "object" && part !== null && "type" in part && part.type === "reasoning" && "reasoning" in part
  );
}

function isV4SourcePart(part: unknown): part is V4SourcePart {
  return typeof part === "object" && part !== null && "type" in part && part.type === "source" && "source" in part;
}
...
function convertV4ToolInvocationToV5ToolUIPart(toolInvocation: ToolInvocation): ToolUIPart {
  return {
    ...
    input: "input" in toolInvocation ? toolInvocation.input : undefined,
    output: toolInvocation.state === "result" && "output" in toolInvocation ? toolInvocation.output : undefined,
...
// Message conversion
function createBaseMessage(msg: V4Message | MyUIMessage, index: number): Pick<MyUIMessage, "id" | "role"> {
  return {
    id: msg.id || `msg-${index}`,
...
function convertV5ToolUIPartToV4ToolInvocation(part: ToolUIPart): ToolInvocation {
  const state =
    part.state === "input-streaming" ? "partial-call" : part.state === "input-available" ? "call" : "result";
  ...
  if (state === "result" && part.output !== undefined) {
    return { ...base, state: "result" as const, output: part.output };
  }
